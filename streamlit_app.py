...
import config
from api_client import USE_MOCK_MODE, process_query, process_query_with_context
from rag import load_rag_index, retrieve_from_rag, save_rag_index
...
st.markdown(
    """<div class="header-container">
        ...
        <p class="header-subtitle">
            Advanced Biomedical Engineering Intelligence with RAG Technology
        </p>
    </div>""",
    unsafe_allow_html=True,
)
    ...
    if st.session_state.show_metrics:
        ...
        with metrics_col3:
            TOOL_COUNT = (
                ...
                if hasattr(config, "TOOL_DEFINITIONS")
                else 0
            )
            st.markdown(
                f"""<div class="metric-card">
...
with st.expander("🎨 Advanced Features", expanded=False):
    ...
    with col_feat1:
        st.session_state.show_metrics = st.checkbox(
            "Show Metrics Dashboard", value=st.session_state.show_metrics
        )

    with col_feat2:
        st.select_slider(
            "Animation Speed", options=["Slow", "Normal", "Fast"], value="Normal"
        )
...
with col_voice:
    st.button("🎤 Voice", use_container_width=True, disabled=True)
        ...
        try:
            ...
            if st.session_state.conversation_history:
                response, new_history = process_query_with_context(
                    user_input, st.session_state.conversation_history[-10:]
                )
...
# Footer
st.markdown("---")
P_STYLE = "font-size: 0.9rem;"
st.markdown(
    f'''<div style="text-align: center; color: #6c757d; padding: 2rem 0;">
        <p>Built with ❤️ for the Biomedical Engineering Community</p>
        <p style="{P_STYLE}">Powered by Grok-4 AI • RAG Technology • Advanced Biomedical Tools</p>
    </div>''',
    unsafe_allow_html=True,
)